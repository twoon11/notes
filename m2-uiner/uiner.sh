...
dataFile="./data.json"		# Полный или относительный путь и имя файла с входными данными в json
...
xargs -P ${maxProcs} -a ${taskFile} \
		...
		--process-slot-var=$outputDir -i sh -c \
				"echo {}; jq --raw-output '.[]|.orderAwaitingPaymentEvent|select(.orderID==\"{}\")|.stateFeeBills|.[]|.uin' \
				${dataFile} > ${outputDir}/{}.txt"
